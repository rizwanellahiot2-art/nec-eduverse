...
type Conversation = { id: string; school_id: string; student_id: string; status: string; created_at: string };
...
export function HrSupportModule() {
  ...
  const selectedLabel = useMemo(() => (selected ? `${selected.student_id} • ${selected.status}` : "Select a ticket"), [selected]);
          ...
          <Table>
            ...
            <TableBody>
              {conversations.map((c) => (
                ...
                >
                  <TableCell className="font-medium">{c.student_id}</TableCell>
